...
import termnsES from "../JSON/termsES.json"
import termsEN from "../JSON/termsEN.json"
...
const Terms = () => {
    ...
    const terms = language === "en" ? termsEN : termnsES;
    ...
    return(
        <div className={`contenedor ${dark ? 'dark' : 'light'}`}>
            <div className={`width-terms-privacy , margin-bot ${dark ? "dark" : "light"}`}>
                ...
                <h1 className="title , center , margin-t-b">{terms[language].title}</h1>
                <span className="texts , bold, margin-bot">{terms[language].date}</span>
                <p className="texts , margin-bot">{terms[language].disclaimer}</p>
                <span className="texts , aling-self-center , margin-bot">{terms[language].privacy}
                    <Link className="links" to="/privacy" target="_blank">{language === "en" ? "Privacy Policy" : " Política de Privacidad"}</Link>
                </span>
